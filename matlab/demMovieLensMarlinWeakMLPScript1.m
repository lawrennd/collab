function [] = demMovieLensMarlinWeakMLPScript1(substract_mean, partNo_v, latentDim_v,iters, inverted)
% DEMMOVIELENSMARLINWEAKMLPSCRIPT1 MLP covariance on Marlin's weak Movielens partitions.
% FORMAT
% DESC Try collaborative filtering with the MLP covariance 
% on the Movielens data for Marlin's partitions for weak generalization.
...
for i_latent=1:length(latentDim_v)
  ...
  for i_part=1:length(partNo_v)
    ...
    saveResults = [capName,'_mlp_inverted_',num2str(inverted),'_norm_',num2str(substract_mean),'_',num2str(q),'_',num2str(partNo),'_iters_',num2str(iters),'.mat'];
