function [] = RecomputeMovieLens10MWeakPartialScript1(substract_mean, partNo_v, latentDim_v,iters, inverted)
% RECOMPUTEMOVIELENS10MWEAKPARTIALSCRIPT1 Recompute the test error for the 10M Movielens database
% where the weak movielens experiment
%
  % RecomputeMovieLens10MWeakPartialScript1(substract_mean, partNo_v,
...
saveResults = ['dem',capName,num2str(experimentNo),'_Iters',num2str(iters),'.mat'];
...
saveResults = ['dem',capName,num2str(experimentNo),'_Iters',num2str(iters),'_error.mat'];
        ...
        save(saveResults, 'model', 'L2_error','NMAE_error','NMAE_round_error');
    end
end
