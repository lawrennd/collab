...
% RBFADDITIONALKERNGRADIENT Gradient of RBF kernel's parameters.
%
%	Description:
%
%	G = RBFADDITIONALKERNGRADIENT(KERN, X, PARTIAL) computes the gradient of
...
%
%	G = RBFADDITIONALKERNGRADIENT(KERN, X1, X2, PARTIAL) computes the derivatives
...
%
%	See also
%	% SEEALSO RBFADDITIONALKERNPARAMINIT, KERNGRADIENT, RBFADDITIOANLKERNDIAGGRADIENT, KERNGRADX


%	Copyright (c) 2009 Raquel Urtasun
% 	rbfKernGradient.m version 1.0



% The last argument is covGrad
if nargin < 4
  [k, dist2xx] = rbfAdditionalKernCompute(kern, x);
else
  [k, dist2xx] = rbfAdditionalKernCompute(kern, x, varargin{1});
end
